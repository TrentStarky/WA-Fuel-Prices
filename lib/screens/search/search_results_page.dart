...
class SearchResultsPage extends StatefulWidget {
  const SearchResultsPage(
      {Key? key, required this.fuelStationsTodayFuture, required this.fuelStationsTomorrowFuture, required this.searchParams})
      : super(key: key);
...
class _SearchResultsPageState extends State<SearchResultsPage> with TickerProviderStateMixin {
  ...
  void initState() {
    Future.wait([widget.fuelStationsTodayFuture, widget.fuelStationsTomorrowFuture]).then((value) {
      //prevent setState call if page is popped while still loading
      if (mounted) {
        setState(() {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
              ...
              icon: (_favourite) ? const Icon(Icons.star) : const Icon(Icons.star_border_outlined),
              onPressed: () {
        ...
        builder: (cxt, con) => Column(
          children: [
            SizedBox(
              ...
              child: Container(
                decoration: const BoxDecoration(border: Border(bottom: BorderSide()), color: ThemeColor.lightBackground),
                child: TabBar(
  ...
  Widget _buildTodaySearch(BoxConstraints con) {
  ...
  Widget _buildTomorrowSearch(BoxConstraints con) {
    return (fuelStationsTomorrow == null)
        ...
        : (fuelStationsTomorrow!.isNotEmpty)
            ? ListView.separated(
  ...
  Widget _buildTile(BoxConstraints con, int index, FuelStation fuelStation) {
    return InkWell(
        onTap: () =>
            Navigator.push(context, TransitionRouter.rightToLeft(SingleStationPage(station: fuelStation))),
        child: Padding(
...
class _CustomPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    paint.strokeWidth = 3;
    canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(15.0)), paint);
  }
}
