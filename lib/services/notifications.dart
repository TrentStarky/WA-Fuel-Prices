...
class Notifications {
  ...
  static Future<void> executeBackgroundTask() async {
    ...
    List<Map> favouriteDBList = await database.rawQuery(
        'SELECT * FROM ${Resources.dbFavourites} WHERE ${Resources.dbPushNotification} IN (?, ?)',
        [1, 2]);
    ...
    for (Map favourite in favouriteDBList) {
      ...
      final todayStations = await FuelWatchService.getFuelStationsToday(
          favouriteInstance.searchParams);
      favouriteInstance.addTodayStations(todayStations);

      final tomorrowStation = await FuelWatchService.getFuelStationsTomorrow(
          favouriteInstance.searchParams);
  ...
  static Future<void> showNotification(
    ...
    const AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails('0', 'Fuel Notifications',
            'Notifications of saved fuel searches when tomorrows\' prices are available.',
            groupKey: 'FuelNotification', playSound: false);
    const IOSNotificationDetails iosPlatformChannelSpecifics =
        IOSNotificationDetails(threadIdentifier: 'wa-fuel');
    ...
    await notificationsPlugin.show(
        ...
        'Today: $todayPrice - $changeIcon Tomorrow: $tomorrowPrice',
  ...
  static Future<void> showGroupNotification(
    ...
    AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails('0', 'Fuel Notifications',
            ...
            groupKey: 'FuelNotification',
            setAsGroupSummary: true);
    NotificationDetails platformChannelSpecifics =
        NotificationDetails(android: androidPlatformChannelSpecifics);
    await notificationsPlugin.show(
        0,
        'WA Fuel Prices - ${dateFormat.format(DateTime.now())}',
        '',
        platformChannelSpecifics);
  }
}
